...
Create a Java class named WebConfig to configure the Spring Boot application. Enable CORS to allow requests from the React frontend. Set the port to 8080 (or another suitable port). Use annotations for configuration.
...
WebConfig class is created and configures the Spring Boot application as described in the PRD. CORS is enabled, and the port is set correctly.
